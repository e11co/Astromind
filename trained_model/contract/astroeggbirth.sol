...
contract AstroEggBirth is EggBase{
    ...
    /// @dev Have input characters, current time, and current caller address to generate random numbers
    function _generateRandomDna(string _str) private view returns (uint) {
        uint rand =uint(keccak256(uint(keccak256(_str)) + uint(keccak256(now)) + uint(keccak256(msg.sender))));
        return uint(rand % dnaModulus);
    }
    
    
    ///@dev 
    function createRandomEgg(string _name) public onlyAdmin whenNotPaused {
    ...
    //set cooldownTime 
    function setCooldownTime(uint _time) public onlyAdmin whenNotPaused {
        cooldownTime = _time;
    ...
    function getEggInfo(uint _eggId) public view returns(
        ...
        uint32 shipPrice,
        uint dna) {
            return (eggs[_eggId].name,eggs[_eggId].owner,eggs[_eggId].shipFlag,eggs[_eggId].collectionCount,eggs[_eggId].likeCount,eggs[_eggId].shipPrice,eggs[_eggId].dna);           
        }
}